...
import { MuuSounds } from '../const/yutumu-muu-sounds';
import { MuuSteps } from '../const/yutumu-muu-steps';
import { tutorialQueue } from './yutumu-intro-tutorials';
  ...
  if (tutorial) {
    ...
    tour.onafterchange(function (this: any) {
      ...
      setTimeout(() => {
        ...
        audio.play().catch((err: Error) => console.error("Error playing sound:", err));
      }, 100);
    });

    tour.start();
  } else {
    console.log("No tutorial available for this page.");
  }
}
